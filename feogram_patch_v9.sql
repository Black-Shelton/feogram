...
-- ════════════════════════════════════════════════════
-- PATCH v9 — nick color in messages + contest type
-- ════════════════════════════════════════════════════
alter table public.messages add column if not exists sender_nick_color text;
alter table public.messages add column if not exists text_style         text;

-- Расширяем тип сообщений для конкурсов и голосований
alter table public.messages drop constraint if exists messages_type_check;
alter table public.messages add constraint messages_type_check
  check (type in ('text','gift','sticker','gif','system','image','video','audio','file','forward','contest','poll','voice','reaction_paid'));
